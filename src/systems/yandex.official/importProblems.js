...
function printResults(inserted) {
  ...
  for (let insertedProblem of inserted) {
    ...
    message += `| <a target="_blank" href='/problems/${insertedProblem.id}'>${insertedProblem.title}</a> (<a target="_blank" href='https://official.contest.yandex.ru/contest/${parsedProblemIdentifier.contestNumber}/problems/${parsedProblemIdentifier.symbolIndex}'>yandexOfficial</a>) | добавлена |
    `;
  }
  if (!inserted.length) {
    message += `| - | - |
